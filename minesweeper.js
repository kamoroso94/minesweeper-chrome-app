...
chrome.storage.sync.get(["custom","difficulty","marks","zoom"],function(options) {
...
window.addEventListener("load",function() {
	...
	zoom.addEventListener("change",function() {
		...
		chrome.storage.sync.set({zoom:this.value});
		resize();
	});
	difficulty.addEventListener("change",function() {
		if(this.value=="custom") {
			...
			chrome.storage.sync.set({custom:false,difficulty:this.value});
			load();
		}
	});
	...
	marks.addEventListener("change",function() {
		...
		chrome.storage.sync.set({marks:this.checked});
	});
	...
	document.getElementById("wiki").addEventListener("click",function() {
		window.open("http://www.minesweeper.info/wiki/Strategy");
	});
	...
	customDialog.querySelector("form").addEventListener("submit",function(e) {
		...
		chrome.storage.sync.set({custom:true,difficulty:config.difficulty});
		hideDialog("custom-dialog");
		load();
	});
	document.getElementById("new-game").addEventListener("click",load);
...
function resize() {
	...
	w = game.style.zoom*rect.width;
	h = menu.getBoundingClientRect().height+game.style.zoom*rect.height;
	app.innerBounds.setSize(w,h);
}
